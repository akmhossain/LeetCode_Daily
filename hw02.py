...
def num_eights(num: int) -> int:
    ...
    True
    """
    # base case or negate value
    if num < 0:
        return num_eights(-num)
    if num == 0:
        return 0
...
def digit_distance(num: int) -> int:
    ...
    True
    """
    # not num == 0, adds extra step
    if num < 10:
        return 0
    
    # num % 10 is last digit num//10 % 10 is 2nd to last
...
def interleaved_sum(num: int, f_odd, f_even) -> int:
    ...
    def odd(n, f_odd):
        if n == 1:
            return f_odd(1)
        return even(n-1, f_even) + f_odd(n)
    
    def even(n, f_even):
        return odd(n-1, f_odd) + f_even(n)

    return odd(num, f_odd)
